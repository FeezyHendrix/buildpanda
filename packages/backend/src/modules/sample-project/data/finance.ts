import type { DeferredUpdate, SampleProjectContext, SampleProjectDataset } from "../types.ts";
...
const TASK_COLUMNS = [
  { key: "todo", name: "To do", status: "todo", position: 0 },
  { key: "in_progress", name: "In progress", status: "in_progress", position: 1 },
  { key: "done", name: "Done", status: "done", position: 2 },
];

export function financeSelectionDecisions(ctx: SampleProjectContext): DeferredUpdate[] {
  return SELECTIONS.flatMap((selection, idx) =>
    selection.chosenIndex === null
      ? []
      : [
          {
            table: "project_selections",
            where: { id: ctx.id(`sel_${idx}`) },
            patch: {
              chosen_option_id: ctx.id(`selopt_${idx}_${selection.chosenIndex}`),
...
export function financeProjectPatch(): Record<string, unknown> {
  return {
...
export function buildFinanceSampleData(ctx: SampleProjectContext): SampleProjectDataset {
  const now = new Date().toISOString();
  const projectId = ctx.projectId;
  const buildingId = ctx.buildingId;

  const materialsCatalog: Record<string, unknown>[] = [];
  const materialsStock: Record<string, unknown>[] = [];
  const materialLedgerEntries: Record<string, unknown>[] = [];

  MATERIALS.forEach((material, idx) => {
    const catalogId = ctx.id(`mcat_${idx}`);
  ...
  const invoiceLineItems: Record<string, unknown>[] = [];
  const invoicePayments: Record<string, unknown>[] = [];

  for (const invoice of INVOICES) {
    const invoiceId = ctx.id(`inv_${invoice.number}`);
    ...
    invoice.lineItems.forEach((item, idx) => {
      invoiceLineItems.push({
        id: ctx.id(`invl_${invoice.number}_${idx}`),
  ...
  const purchaseOrderItems: Record<string, unknown>[] = [];

  for (const po of PURCHASE_ORDERS) {
    const poId = ctx.id(`po_${po.number}`);
    purchaseOrders.push({
      id: poId,
      project_id: projectId,
      po_number: po.number,
    ...
    po.items.forEach((item, idx) => {
      purchaseOrderItems.push({
        id: ctx.id(`poi_${po.number}_${idx}`),
  ...
  const boardId = ctx.id("tb_site-tasks");
  const columnIdByStatus = new Map(TASK_COLUMNS.map((c) => [c.status, ctx.id(`tc_${c.key}`)]));

  const selections: Record<string, unknown>[] = [];
  const selectionOptions: Record<string, unknown>[] = [];

  SELECTIONS.forEach((selection, idx) => {
    const selectionId = ctx.id(`sel_${idx}`);
    // chosen_option_id stays null here: project_selections -> project_selection_options is a
    // circular FK, so the choice can only be linked after the options rows exist.
    selections.push({
      id: selectionId,
      project_id: projectId,
    ...
    selection.options.forEach((opt, optIdx) => {
      selectionOptions.push({
        id: ctx.id(`selopt_${idx}_${optIdx}`),
  ...
  return [
    {
      table: "suppliers",
      rows: SUPPLIERS.map((supplier, idx) => ({
        id: ctx.id(`sup_${idx}`),
        project_id: projectId,
        name: supplier.name,
    ...
    { table: "materials_catalog", rows: materialsCatalog },
    { table: "materials_stock", rows: materialsStock },
    { table: "material_ledger_entries", rows: materialLedgerEntries },
    {
    ...
    { table: "project_invoices", rows: invoices },
    { table: "project_invoice_line_items", rows: invoiceLineItems },
    { table: "invoice_payments", rows: invoicePayments },
    { table: "purchase_orders", rows: purchaseOrders },
    { table: "purchase_order_items", rows: purchaseOrderItems },
    {
      table: "payment_claims",
      rows: PAYMENT_CLAIMS.map((claim) => ({
        id: ctx.id(`pc_${claim.number}`),
        project_id: projectId,
    ...
    {
      table: "task_columns",
      rows: TASK_COLUMNS.map((c) => ({
        id: columnIdByStatus.get(c.status)!,
        board_id: boardId,
        name: c.name,
        status: c.status,
        position: c.position,
    ...
    },
    {
      ...
      rows: TASKS.map((task, idx) => ({
        ...
        building_id: buildingId,
        board_id: boardId,
        column_id: columnIdByStatus.get(task.columnStatus)!,
        title: task.title,
    ...
    },
    {
      ...
      rows: RFIS.map((rfi) => ({
        ...
        official_responded_at:
          rfi.daysAgoResponded !== null && rfi.daysAgoResponded !== undefined ? isoDaysAgo(rfi.daysAgoResponded) : null,
        cost_impact: 0,
        schedule_impact: 0,
        change_request_id: null,
        reopened_count: 0,
    ...
    { table: "project_selections", rows: selections },
    { table: "project_selection_options", rows: selectionOptions },
    {
      table: "risk_factors",
      rows: RISKS.map((risk, idx) => ({
        id: ctx.id(`rf_${idx}`),
        project_id: projectId,
        title: risk.title,
    ...
    },
    {
      ...
      rows: FINANCE_EVENTS.map((event, idx) => ({
        id: ctx.id(`fev_${idx}`),
        project_id: projectId,
        type: event.type,
        actor_id: null,
