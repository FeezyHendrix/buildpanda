import type { MeasuredItem, TakeoffResult } from "../dwg-takeoff/types.ts";
...
// The DWG engine measures the representative floor plan and multiplies it by
// the identical floors it found; truth is stated per sheet. So the score reads
// the items measured on the representative plan, undoes the multiplier, and
// sums lines of the same trade (walls arrive as one line per thickness).
export function scoreDwg(truth: Truth, result: Pick<TakeoffResult, "items" | "sheets"> | MeasuredItem[]): Scored[] {
  ...
  const sheets = Array.isArray(result) ? [] : (result.sheets ?? []);
  const representative = sheets.find((s) => s.kind === "floor-plan" && s.multiplier >= 1) ?? sheets.find((s) => s.kind === "floor-plan");
  const items = representative ? allItems.filter((i) => i.sheetId === undefined || i.sheetId === representative.id) : allItems;
  const perSheet = (i: MeasuredItem) => i.quantity / (i.multiplier && i.multiplier > 1 ? i.multiplier : 1);
  const byTrade = (pred: (i: MeasuredItem) => boolean): Candidate | null => {
    const matches = items.filter(pred);
    if (matches.length === 0) return null;
    const quantity = Math.round(matches.reduce((a, i) => a + perSheet(i), 0) * 100) / 100;
    const worst = matches.some((i) => i.confidence === "low") ? "low" : matches.some((i) => i.confidence === "medium") ? "medium" : "high";
    return { quantity, unit: matches[0]!.unit, confidence: worst, basis: matches.map((i) => i.basis).join(" | ") };
  };
  const out: Scored[] = [];
  const ext = els.find((e) => e.element === "walls-external");
  const int = els.find((e) => e.element === "walls-internal");
  if (ext && int) {
    const walls = byTrade((i) => /wall/i.test(i.trade) && i.unit === "m2");
  ...
  for (const [kind, pattern] of [
    ...
    if (el?.count !== undefined) out.push(score(el, "count", el.count, byTrade((i) => pattern.test(i.trade) && i.unit === "nr")));
  }
  const floor = els.find((e) => e.element === "floor-area");
  if (floor?.areaM2 !== undefined) out.push(score(floor, "areaM2", floor.areaM2, byTrade((i) => /floor area|room/i.test(i.trade) && i.unit === "m2")));
  return out;
}
