...
import { measureWallRuns, wallSegments } from "./walls.ts";
...
test("rooms flood fill inside the wall faces from their labels; door openings are sealed by the leaf and swing", () => {
  ...
  const walls = wallSegments(doc, sheet, map);
  const rooms = measureRooms(doc, sheet, walls, measureWallRuns(walls, units.scaleToMm).seals, units, map);
...
test("the stated-area note says agree, differ, or which number of flats the sum fits", () => {
  ...
  const off = statedAreaNote(result({ totalM2: 240, statedM2: 163, perUnit: true, units: 2, unmeasured: [{ name: "TERRACE", why: "leaked" }, { name: "TERRACE", why: "leaked" }] }), "DWG-01") ?? "";
  assert.match(off, /× 2 labelled kitchens = 326 m² \(differ by 26%/);
  assert.match(off, /Not measured: TERRACE \(leaked\) ×2/);
