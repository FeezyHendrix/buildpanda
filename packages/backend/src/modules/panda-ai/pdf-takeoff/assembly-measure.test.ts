...
function harness() {
  ...
  const manual = {
    ...
    insert: async (_bill: PreconBillRow, line: Line, actor: string) => {
      got.lines.push(line);
      n += 1;
      return { id: `pbr_${n}`, bill_id: bill.id, description: line.description, unit: line.unit, qty: line.gross * line.typical, rate: line.rate ?? null, measurement_basis: line.basis, element_group: line.elementGroup, code: line.code ?? null, version: 1, verified_by: actor } as unknown as PreconBoqRowRow;
    },
