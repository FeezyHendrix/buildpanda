...
export function MentionDropdown({
  ...
  return (
    <div className="absolute bottom-full left-0 mb-2 max-h-48 w-64 overflow-y-auto rounded-lg border border-line-hair bg-white p-1 shadow-lg">
      {filtered.map((m) => (
        <button
        ...
        >
          {m.kind === "user" ? (
            <>
              ...
              <span className="truncate font-medium text-ink">{m.member.name}</span>
            </>
          ) : (
            <>
              ...
              <span className="min-w-0">
                ...
                <span className="block truncate text-xs text-ink-muted">{m.helper}</span>
              </span>
            </>
          )}
        </button>
