...
export function SheetPane({
  ...
  return (
    <section className="relative flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-lg border border-line-hair bg-white">
      <div className="flex items-center gap-2 border-b border-line-hair px-3 py-2">
        <span className="rounded bg-surface-alt px-1.5 py-0.5 text-xs font-medium uppercase text-ink-muted">
          {label}
        </span>
        ...
        <select
          ...
          className={cn(INPUT_SM_CLASS, "min-w-0 max-w-44 font-medium")}
        >
          {sheets.map((s, i) => (
        ...
        <span className="ml-auto text-xs text-gray-500">{sheet.revision}</span>
      ...
      <div className="relative min-h-0 flex-1 overflow-auto bg-gray-100 p-4">
        <div className="mx-auto w-fit min-w-full">
          <SheetImage
            ...
            className="mx-auto w-full max-w-3xl rounded border border-line bg-white shadow-md"
          />
        </div>
        {locked && (
      ...
      <div className="flex items-center justify-center gap-2 border-t border-line-hair px-3 py-1.5">
        <IconBtn
