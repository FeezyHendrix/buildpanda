...
interface TableProps extends HTMLAttributes<HTMLTableElement> {
  ...
  /** Pull the table out to the page edges so its cell padding becomes the gutter. */
  bleed?: boolean;
}
...
function Table({ className, wrapperClassName, bleed = false, children, ...props }: TableProps) {
  return (
    <div className={cn("overflow-x-auto", bleed && "-mx-6", wrapperClassName)}>
...
/** An in-table section heading row. */
function TableSectionRow({ colSpan, children }: { colSpan: number; children: ReactNode }) {
  return (
    <tr className="border-b border-[#F0F0F0] bg-[#F6F6F6]">
      <td colSpan={colSpan} className="px-6 py-2 text-[11px] font-semibold uppercase tracking-wider text-black-300">
        {children}
      </td>
    </tr>
  );
