...
interface CountrySelectProps {
  ...
  className?: string;
  /** Show only the 2-letter ISO code in the trigger (flag + name still shown in the dropdown) */
  codeOnly?: boolean;
  placeholder?: string;
}
...
function CountrySelect({ value, onChange, className, codeOnly = false, placeholder }: CountrySelectProps) {
  return (
    <Combobox.Root
      items={countries}
      value={value}
      ...
      >
        <Combobox.Value>
          {(selected: Country | null) =>
            selected ? (
              <span>{codeOnly ? selected.code : `${selected.flag} ${selected.name}`}</span>
            ) : (
              <span className="text-[#B0B0B0]">{placeholder ?? "Select country"}</span>
            )
          }
        </Combobox.Value>
        ...
        <Combobox.Positioner align="start" sideOffset={4} className="z-[60]">
          ...
          >
            ...
            <div className="bg-grey-50 pt-1">
              <Combobox.Empty className="empty:hidden px-4 py-3 text-[13px] text-[#767676]">
                No countries found.
              </Combobox.Empty>
