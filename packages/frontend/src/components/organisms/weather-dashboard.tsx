  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5">
        <span className="text-[18px] leading-none">{CONDITION_GLYPH[day.condition]}</span>
        <span className={cn("text-[12px]", isToday ? "font-semibold text-white" : "text-white/85")}>
          {dayLabel(day.date, index)}
        </span>
      </div>
      <div className="flex items-center gap-2 tabular-nums">
        {day.precipitationMm > 0 && (
          <span className="text-[10px] font-medium text-white/60">{day.precipitationMm}mm</span>
        )}
        <span className="text-[12px] font-bold text-white">{Math.round(day.temperatureMaxC)}°</span>
        <span className="text-[11px] text-white/60">{Math.round(day.temperatureMinC)}°</span>
      </div>
    </div>
  );
}
...
export function WeatherDashboard({ projectId }: { projectId: string }) {
  ...
  return (
    <section aria-label="Weather" className="grid grid-cols-1 gap-4 lg:grid-cols-4">
      <Card
        padding="md"
      ...
      >
        <div className="flex items-start justify-between">
          <div>
            ...
            {forecast.data?.locationName && (
              <p className="mt-0.5 text-[14px] font-semibold">{forecast.data.locationName}</p>
            )}
          </div>
          {current && <span className="text-[36px] leading-none drop-shadow-sm">{CONDITION_GLYPH[current.condition]}</span>}
        </div>
        ...
        {current && (
          <div className="mt-2">
            ...
            <p className="mt-1 text-[12px] text-white/80">{current.condition}</p>
            <div className="mt-2 flex gap-4 text-[11px] text-white/70">
              <span>
                Wind <span className="font-semibold text-white">{current.windKph} km/h</span>
              </span>
              <span>
                Rain <span className="font-semibold text-white">{current.precipitationMm} mm</span>
              </span>
            </div>
          </div>
        )}
      ...
      <Card padding="md" className="rounded-[16px] border-none bg-[#F8F8F8] p-5 lg:col-span-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="flex size-7 items-center justify-center rounded-full bg-primary/[0.1]">
              <ReactSVG src={icons.aiVerify} className="[&_svg]:size-4" />
            </span>
            <p className="text-[13px] font-semibold text-primary">Panda AI · Weather impact</p>
          </div>
        ...
        {analysis.isLoading ? (
          ...
          <div className="mt-4 flex flex-col gap-4">
            <div>
              ...
              {analysis.data.impact && (
                <p className="mt-1.5 max-w-3xl text-[13px] leading-relaxed text-black-300">
                  {analysis.data.impact}
                </p>
              )}
            </div>
                ...
                {analysis.data.scheduleImpact && (
                  <div className="rounded-[12px] bg-white p-3.5">
                    <div className="mb-1 flex items-center gap-1.5">
                      <ReactSVG src={icons.calendarSearch} className="[&_svg]:size-3.5 [&_svg]:opacity-60" />
                      <p className="text-[11px] font-semibold uppercase tracking-wide text-black-300">
                        Schedule impact
                      </p>
                    </div>
                    <p className="text-[13px] leading-snug text-black-500">{analysis.data.scheduleImpact}</p>
                  </div>
                )}
                {analysis.data.costImpact && (
                  <div className="rounded-[12px] bg-white p-3.5">
                    <div className="mb-1 flex items-center gap-1.5">
                      <ReactSVG src={icons.card} className="[&_svg]:size-3.5 [&_svg]:opacity-60" />
                      <p className="text-[11px] font-semibold uppercase tracking-wide text-black-300">
                        Cost impact
                      </p>
                    </div>
                    <p className="text-[13px] leading-snug text-black-500">{analysis.data.costImpact}</p>
                  </div>
                )}
              </div>
            )}
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                  {analysis.data.recommendations.map((rec, idx) => (
                    <div
                      key={rec}
                      className="flex items-start gap-2 rounded-[12px] border border-primary/15 bg-primary/[0.04] p-3"
                    >
                      <span className="flex size-5 shrink-0 items-center justify-center rounded-full bg-primary text-[11px] font-bold text-white">
                        {idx + 1}
                      </span>
                      <p className="text-[12px] leading-snug text-black-500">{rec}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <p className="mt-4 text-[13px] text-black-300">Weather analysis is unavailable right now.</p>
        )}
      </Card>
    </section>
  );
