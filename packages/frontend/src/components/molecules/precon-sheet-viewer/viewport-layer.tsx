...
function ViewportFrame({ viewport, toPx, cssZoom, onRemove, onEdit }: { viewport: SheetViewport; toPx: Props["toPx"]; cssZoom: number; onRemove?: (id: string) => void; onEdit?: (id: string) => void }) {
  ...
  return (
    <g>
      ...
      <text
        ...
        fontSize={fontSize}
        fill={FRAME_COLOR}
        data-region-label={viewport.id}
        className={onEdit ? "pointer-events-auto cursor-pointer select-none hover:fill-primary-600" : "select-none"}
        onClick={(e) => {
          if (!onEdit) return;
          e.stopPropagation();
          onEdit(viewport.id);
      ...
      >
        {onEdit ? <title>Edit this region's label, scale or bounds</title> : null}
        {label}
      </text>
      {onRemove ? (
        <text
 ...
 */
export function ViewportLayer({ widthPx, heightPx, viewports, toPx, cssZoom, dragRect, dragColor = "#004DE7", onRemove, onEdit }: Props) {
  ...
  return (
    <svg className="pointer-events-none absolute left-0 top-0" width={widthPx} height={heightPx} viewBox={`0 0 ${widthPx} ${heightPx}`}>
      {viewports.map((viewport) => (
        <ViewportFrame key={viewport.id} viewport={viewport} toPx={toPx} cssZoom={cssZoom} onRemove={onRemove} onEdit={onEdit} />
      ))}
